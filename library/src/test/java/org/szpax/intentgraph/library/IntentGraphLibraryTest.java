...
class IntentGraphLibraryTest {
        ...
        IntentGraphDelta delta = new IntentGraphDelta(
            ...
            List.of(
                new DeltaOperation(
                    DeltaOperationType.ADD_NODE,
                    "n1",
                    Map.of("id", "n1", "type", "behavior", "name", "Node 1 duplicate", "description", "D1")
                )
            )
        );
    ...
    void testSubgraphExtraction() {
        IntentNode n1 = new IntentNode("n1", NodeType.behavior, "N1", "D1", 
            List.of(), List.of(new EntryPoint(EntryPointKind.REST, "/start")), List.of(), List.of(new NodeReference("n2")), Map.of());
    ...
    void testGenerateDelta() {
        ...
        ChatClient fakeClient = new ChatClient() {
            ...
            public ChatMessage complete(List<ChatMessage> messages, List<ToolDefinition> tools) {
                ...
                assertTrue(messages.stream().anyMatch(m -> m.role() == ChatRole.USER && m.content().contains("logout")));
    ...
    void testGenerateDeltaExceedsToolTurns() {
        ...
        assertThrows(IllegalStateException.class, () -> library.generateDelta("SYSTEM", "anything", graph, loopingClient));
    }
}
